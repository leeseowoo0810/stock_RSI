...
async def api_stocks():
    """10대 주식 실시간 RSI 및 매수 랭킹 반환 (25초 캐시)"""
    now = time.time()
    market_info = get_market_status()

    if CACHE["data"] is not None and (now - CACHE["last_fetched"]) < CACHE["ttl"]:
...
async def serve_index():
    ...
    return JSONResponse(content={"message": "Frontend static file is not ready yet."})
